...
const ListadoCalzados = () => {
  ...
  const fetchCalzados = async () => {
    try {
      ...
      const formattedData = response.data.map((item) => ({
        ...
        tipo_registro: item.tipo_registro.toLowerCase(),
      }));
  ...
  const filteredData = calzados
    .filter((item) => {
      if (filter === "dubitada") return item.tipo_registro === "dubitada";
      if (filter === "indubitada") return item.tipo_registro === "indubitada";
      if (filter === "indubitada_comisaria") return item.tipo_registro === "indubitada_comisaria";
      return true;
    })
    .filter((item) =>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div className="flex space-x-2">
            ...
            <button onClick={() => setFilter("indubitada_comisaria")} className={`px-4 py-2 rounded-lg ${filter === "indubitada_comisaria" ? "bg-purple-600 text-white" : "bg-gray-200"}`}>Indubitadas Comisaría</button>
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
              ...
              <div className="grid grid-cols-1 gap-4">
                {["marca", "modelo", "talle", "alto", "ancho", "colores", "categoria", "tipo_registro"].map((field) => (
                  <div key={field}>
